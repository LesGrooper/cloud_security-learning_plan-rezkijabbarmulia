...
- Deploy WireGuard VPN server per environment (DEV / UAT / PROD) with role-based peer access control
...
#### Step 4: WireGuard VPN — Per-Environment Clustering (DEV / UAT / PROD)

> WireGuard is a modern, lightweight VPN (kernel-level, ~4 000 LoC) using Curve25519 + ChaCha20-Poly1305.
> Each environment gets its **own WireGuard server** on a dedicated EC2 instance, with separate peer configs and CIDRs.
> No shared key material crosses environment boundaries.

---

##### Environment Architecture

| Environment | WireGuard Server | VPN CIDR | EC2 Size | Endpoint |
|-------------|-----------------|----------|----------|---------|
| **PROD** | `vpn-prod` (ap-southeast-1a, private subnet) | 10.200.0.0/24 | t3.small | `vpn-prod.rentdrive.internal:51820` |
| **UAT** | `vpn-uat` (ap-southeast-1b, private subnet) | 10.201.0.0/24 | t3.micro | `vpn-uat.rentdrive.internal:51820` |
| **DEV** | `vpn-dev` (ap-southeast-1c, private subnet) | 10.202.0.0/24 | t3.micro | `vpn-dev.rentdrive.internal:51820` |

Each server is in its own **Security Group** — peers from one environment cannot reach another.

---

##### Step 4a: Provision WireGuard Server (repeat per environment)

> **Note:** This script runs **on the Linux EC2 instance** via `--user-data` at launch.  
> It uses bash because the EC2 instance runs Ubuntu/Amazon Linux 2.  
> You launch the EC2 instance from PowerShell in Step 4b; this script executes automatically on boot.

```bash
#!/bin/bash
# EC2 user-data script — executes on Ubuntu/Amazon Linux 2 at first boot
# Variable ENV must be set: prod | uat | dev

ENV="prod"          # change to uat or dev when running on that host
VPN_CIDR_MAP_prod="10.200.0.0/24"
VPN_CIDR_MAP_uat="10.201.0.0/24"
VPN_CIDR_MAP_dev="10.202.0.0/24"
eval "VPN_CIDR=\$VPN_CIDR_MAP_${ENV}"

SERVER_IP="${VPN_CIDR%.*}.1"   # first host: 10.200.0.1 / 10.201.0.1 / 10.202.0.1

# ── Install WireGuard ─────────────────────────────────────────────────────────
sudo apt-get update -qq
sudo apt-get install -y wireguard wireguard-tools

# ── Generate server keypair (stored securely, never committed) ────────────────
umask 077
...
# Push public key to SSM Parameter Store (peers can fetch it; private key stays on disk)
aws ssm put-parameter \
  --name "/rentdrive/vpn/${ENV}/server_public_key" \
  --value "$SERVER_PUBKEY" \
...
# Enable IP forwarding for VPN tunnel routing
...
sudo wg show           # verify: interface wg0 is up, listening on :51820

echo "WireGuard server (${ENV}) running — endpoint: ${SERVER_IP}:51820"
```

---

##### Step 4b: Launch EC2 WireGuard Server per Environment (AWS CLI)

```powershell
# ── Security Group per environment ───────────────────────────────────────────
$Region = "ap-southeast-1"

foreach ($Env in @("prod", "uat", "dev")) {
    switch ($Env) {
        "prod" { $VpcId = "vpc-prod-xxxx"; $Subnet = "subnet-prod-private" }
        "uat"  { $VpcId = "vpc-uat-xxxx";  $Subnet = "subnet-uat-private"  }
        "dev"  { $VpcId = "vpc-dev-xxxx";  $Subnet = "subnet-dev-private"  }
    }

    # Create dedicated SG for WireGuard server in this env
    $SgId = aws ec2 create-security-group `
        --group-name "rentdrive-wg-$Env" `
        --description "WireGuard VPN server — $Env" `
        --vpc-id $VpcId `
        --query GroupId --output text

    # Allow WireGuard UDP from internet (peers connect here)
    aws ec2 authorize-security-group-ingress --group-id $SgId `
        --protocol udp --port 51820 --cidr 0.0.0.0/0

    # Access via SSM Session Manager — no SG rule needed for port 22

    # Launch WireGuard server EC2
    $InstanceId = aws ec2 run-instances `
        --image-id ami-0c55b159cbfafe1f0 `
        --instance-type t3.micro `
        --subnet-id $Subnet `
        --security-group-ids $SgId `
        --iam-instance-profile Name=WireGuardSSMRole `
        --user-data file://wg-server-setup.sh `
        --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=vpn-$Env},{Key=Environment,Value=$Env},{Key=ManagedBy,Value=aws-cli}]" `
        --query "Instances[0].InstanceId" --output text

    Write-Host "Launched vpn-${Env}: $InstanceId (SG: $SgId)"
}
```

---

##### Step 4c: IAM Role for WireGuard Server (SSM + SSM Parameter access)

```powershell
# Create IAM role — WireGuard servers use this for SSM + key publishing
aws iam create-role `
    --role-name WireGuardSSMRole `
    --assume-role-policy-document '{
        "Version":"2012-10-17",
        "Statement":[{"Effect":"Allow","Principal":{"Service":"ec2.amazonaws.com"},"Action":"sts:AssumeRole"}]
    }'

# Attach SSM managed instance core (Session Manager, no open port 22)
aws iam attach-role-policy `
    --role-name WireGuardSSMRole `
    --policy-arn arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore

# Allow publishing public key to SSM Parameter Store
aws iam put-role-policy `
    --role-name WireGuardSSMRole `
    --policy-name WireGuardKeyPublish `
    --policy-document '{
        "Version":"2012-10-17",
        "Statement":[{
            "Effect":"Allow",
            "Action":["ssm:PutParameter","ssm:GetParameter"],
            "Resource":"arn:aws:ssm:ap-southeast-1:*:parameter/rentdrive/vpn/*"
        }]
    }'

aws iam create-instance-profile --instance-profile-name WireGuardSSMRole
aws iam add-role-to-instance-profile `
    --instance-profile-name WireGuardSSMRole `
    --role-name WireGuardSSMRole
```

---

##### Step 4d: Generate Peer Config per Role + Environment

```powershell
# Run on developer Windows laptop to generate personal WireGuard peer config
# Save as: gen-peer.ps1
# Usage: .\gen-peer.ps1 -Env dev -Username alice -Role engineer
# Prerequisite: WireGuard for Windows installed (https://www.wireguard.com/install/)

param(
    [Parameter(Mandatory)][ValidateSet("prod","uat","dev")][string]$Env,
    [Parameter(Mandatory)][string]$Username,
    [Parameter(Mandatory)][ValidateSet("engineer","dba","devops","readonly")][string]$Role
)

$Region = "ap-southeast-1"

# Fetch server public key from SSM Parameter Store
$ServerPubKey = aws ssm get-parameter `
    --name "/rentdrive/vpn/$Env/server_public_key" `
    --query Parameter.Value --output text `
    --region $Region

# Environment endpoint + DNS map
$EndpointMap = @{
    prod = @{ Endpoint = "vpn-prod.rentdrive.internal:51820"; DNS = "10.200.0.1" }
    uat  = @{ Endpoint = "vpn-uat.rentdrive.internal:51820";  DNS = "10.201.0.1" }
    dev  = @{ Endpoint = "vpn-dev.rentdrive.internal:51820";  DNS = "10.202.0.1" }
}
$ServerEndpoint = $EndpointMap[$Env].Endpoint
$AllowedDns     = $EndpointMap[$Env].DNS

# Role-based AllowedIPs (split-tunnel — only env-specific private ranges)
$AllowedIpsMap = @{
    "prod-engineer" = "10.0.1.0/24"      # App subnet only
    "prod-dba"      = "10.0.3.0/24"      # DB subnet only
    "prod-devops"   = "10.0.0.0/16"      # Full PROD VPC
    "prod-readonly" = "10.0.5.0/24"      # Log/audit subnet only
    "uat-engineer"  = "10.1.1.0/24"
    "uat-dba"       = "10.1.3.0/24"
    "uat-devops"    = "10.1.0.0/16"
    "dev-engineer"  = "10.2.0.0/16"      # Full DEV VPC — open for devs
    "dev-dba"       = "10.2.0.0/16"
    "dev-devops"    = "10.2.0.0/16"
}
$AllowedIPs = $AllowedIpsMap["$Env-$Role"]

# Generate peer keypair using wg.exe (bundled with WireGuard for Windows)
$PeerPrivKey = (wg genkey)
$PeerPubKey  = ($PeerPrivKey | wg pubkey)
$Preshared   = (wg genpsk)   # optional preshared key for extra PQ resistance

# Write peer config to %USERPROFILE%\.wireguard\
$ConfigDir  = "$env:USERPROFILE\.wireguard"
New-Item -ItemType Directory -Force -Path $ConfigDir | Out-Null
$ConfigPath = "$ConfigDir\$Env-rentdrive.conf"

@"
[Interface]
# $Username @ $Env — $Role
PrivateKey = $PeerPrivKey
Address    = # assigned by admin — see VPN Access Matrix below
DNS        = $AllowedDns
...
Write-Host "Peer config written: $ConfigPath"
Write-Host "Peer public key (send to admin to register on server):"
Write-Host $PeerPubKey
Write-Host "AllowedIPs for this peer: $AllowedIPs"
```

---

##### Step 4e: Register Peer on WireGuard Server (admin task)

> **Note:** Run these commands **on the WireGuard EC2 server** via SSM Session Manager.  
> Open a session first: `aws ssm start-session --target <instance-id>` (from PowerShell)  
> The commands below are bash — executed inside the Linux EC2 session.

```bash
# On the WireGuard EC2 server via SSM Session Manager (Linux bash session)
# Add a new peer (developer) to the running wg0 interface live (no restart)

ENV="dev"
PEER_PUBKEY="<paste peer public key from gen-peer.sh output>"
PEER_PSK="<paste preshared key>"
PEER_IP="10.202.0.10"    # allocate from VPN CIDR: 10.200.x/24=prod, 10.201.x/24=uat, 10.202.x/24=dev

# Add peer live (no service restart required — WireGuard hot-add)
sudo wg set wg0 \
  peer "$PEER_PUBKEY" \
  preshared-key <(echo "$PEER_PSK") \
  allowed-ips "${PEER_IP}/32"

# Persist to config file so it survives server reboot
sudo tee -a /etc/wireguard/wg0.conf > /dev/null <<PEER

[Peer]
# ${PEER_USERNAME} — ${PEER_ROLE} (added $(date +%Y-%m-%d))
PublicKey    = ${PEER_PUBKEY}
PresharedKey = ${PEER_PSK}
AllowedIPs   = ${PEER_IP}/32
PEER

sudo wg show wg0   # verify peer appears in handshake list
```

---

**WireGuard VPN Access Matrix — RentDrive (per Environment)**

| Env | User Group | Peer VPN IP | AllowedIPs (split-tunnel) | Ports | Key Rotation |
|-----|------------|------------|--------------------------|-------|-------------|
| **PROD** | `devops` | 10.200.0.0/26 | Full VPC 10.0.0.0/16 | All private | 90 days |
| **PROD** | `engineer` | 10.200.0.64/26 | App subnet 10.0.1.0/24 | 443, 8443 | 90 days |
| **PROD** | `dba-team` | 10.200.0.128/26 | DB subnet 10.0.3.0/24 | 3306 | 90 days |
| **PROD** | `readonly-audit` | 10.200.0.192/26 | Log subnet 10.0.5.0/24 | 443 | 90 days |
| **UAT** | `devops` | 10.201.0.0/26 | Full UAT VPC 10.1.0.0/16 | All private | 180 days |
| **UAT** | `engineer` | 10.201.0.64/26 | UAT app subnet 10.1.1.0/24 | 443, 8443 | 180 days |
| **UAT** | `dba-team` | 10.201.0.128/26 | UAT DB subnet 10.1.3.0/24 | 3306 | 180 days |
| **DEV** | `all-devs` | 10.202.0.0/24 | Full DEV VPC 10.2.0.0/16 | All private | 365 days |

> No peer in DEV can reach UAT or PROD VPN CIDRs — Security Groups block cross-environment WireGuard traffic at the AWS network level.

---

##### Step 4f: Monitoring — Active Peers + Handshake Age

```bash
# ── Server-side: run on WireGuard EC2 via SSM Session Manager ────────────────
# Check all active peers and last handshake time
sudo wg show wg0 dump | awk '
NR>1 {
  pubkey=$1; allowed=$4; last_handshake=$5; rx=$6; tx=$7
  age = systime() - last_handshake
  status = (age < 180) ? "ACTIVE" : (last_handshake == 0) ? "NEVER" : "STALE"
  printf "%-20s  %-15s  %-8s  rx=%s tx=%s\n", substr(pubkey,1,20), allowed, status, rx, tx
}'

# Publish active peer count to CloudWatch — run as cron every 5 min on server
# crontab entry: */5 * * * * /opt/scripts/wg-metrics.sh
ACTIVE_PEERS=$(sudo wg show wg0 dump | awk 'NR>1 && systime()-$5 < 180 {count++} END{print count+0}')
aws cloudwatch put-metric-data \
  --namespace "RentDrive/WireGuard" \
  --metric-name "ActivePeers" \
...
```powershell
# ── Local admin: create CloudWatch alarm from Windows machine ─────────────────
# CloudWatch alarm: no active PROD peers for > 5 min
aws cloudwatch put-metric-alarm `
    --alarm-name "wg-prod-no-active-peers" `
    --namespace "RentDrive/WireGuard" `
    --metric-name "ActivePeers" `
...
# Run by: On-call Security Engineer (from Windows workstation)

$IncidentId       = "INC-$(Get-Date -Format 'yyyyMMdd')-001"
$AffectedInstance = "i-0123456789abcdef0"   # From GuardDuty finding
$AccountId        = aws sts get-caller-identity --query Account --output text
$Region           = "ap-southeast-1"
$SnsTopic         = "arn:aws:sns:${Region}:${AccountId}:rentdrive-security-alerts"
$QuarantineSg     = "sg-00000000000000000"   # Pre-created: deny all inbound + outbound
$Timestamp        = (Get-Date).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ")

Write-Host "=== INCIDENT RESPONSE INITIATED: $IncidentId — $Timestamp ==="

# ─── STEP 1: PRESERVE EVIDENCE (before touching anything) ───────────────────
Write-Host "[1/5] Creating forensic EBS snapshot (pre-containment state)..."
$VolumeId = aws ec2 describe-instances `
    --instance-ids $AffectedInstance --region $Region `
    --query 'Reservations[0].Instances[0].BlockDeviceMappings[0].Ebs.VolumeId' `
    --output text

aws ec2 create-snapshot `
    --volume-id $VolumeId --region $Region `
    --description "FORENSIC-$IncidentId" `
    --tag-specifications "ResourceType=snapshot,Tags=[{Key=incident-id,Value=$IncidentId},{Key=purpose,Value=forensic-evidence},{Key=do-not-delete,Value=true},{Key=timestamp,Value=$Timestamp}]"

Write-Host "[2/5] Exporting CloudTrail events for affected instance (last 48h)..."
$ForensicFile = "$env:TEMPorensic-cloudtrail-$IncidentId.json"
aws cloudtrail lookup-events `
    --lookup-attributes AttributeKey=ResourceName,AttributeValue=$AffectedInstance `
    --region $Region | Set-Content $ForensicFile
$LineCount = (Get-Content $ForensicFile).Count
Write-Host "CloudTrail export: $LineCount lines saved to $ForensicFile"

# ─── STEP 2: CONTAINMENT ────────────────────────────────────────────────────
Write-Host "[3/5] Applying quarantine security group (deny all traffic)..."
aws ec2 modify-instance-attribute `
    --instance-id $AffectedInstance `
    --groups $QuarantineSg `
    --region $Region
Write-Host "Instance $AffectedInstance: QUARANTINED"

# ─── STEP 3: REVOKE IAM CREDENTIALS ────────────────────────────────────────
Write-Host "[4/5] Suspending IAM role permissions via inline Deny-All policy..."
$ProfileArn = aws ec2 describe-instances `
    --instance-ids $AffectedInstance --region $Region `
    --query 'Reservations[0].Instances[0].IamInstanceProfile.Arn' `
    --output text
$RoleName = $ProfileArn.Split("/")[-1]

aws iam put-role-policy `
    --role-name $RoleName `
    --policy-name "INCIDENT-QUARANTINE-$IncidentId" `
    --policy-document '{"Version":"2012-10-17","Statement":[{"Effect":"Deny","Action":"*","Resource":"*"}]}'
Write-Host "IAM role $RoleName: all permissions SUSPENDED"

# ─── STEP 4: NOTIFY ─────────────────────────────────────────────────────────
Write-Host "[5/5] Publishing incident notification to security team..."
$Message = @"
INCIDENT: $IncidentId
Instance: $AffectedInstance
Region: $Region
Status: CONTAINED
Time: $Timestamp
Forensic snapshot: created
CloudTrail export: $ForensicFile
...
aws sns publish `
    --topic-arn $SnsTopic `
    --subject "SECURITY INCIDENT CONTAINED: $IncidentId" `
    --message $Message

Write-Host ""
Write-Host "=== INCIDENT $IncidentId CONTAINED ==="
Write-Host "Next steps: Attach forensic AMI, mount snapshot read-only, begin memory analysis"
```
...
# ─── Monthly DR Test Script (PowerShell) ─────────────────────────────────────
# Save as: dr-monthly-test.ps1 and run from Windows workstation

$Date       = Get-Date -Format "yyyyMMdd"
$TestInstId = "rentdrive-drtest-$Date"
Write-Host "=== RentDrive Monthly DR Restoration Test — $(Get-Date) ==="
...
Write-Host "Restoring snapshot: $SnapshotId to isolated test environment..."

aws rds restore-db-instance-from-db-snapshot `
    --db-instance-identifier $TestInstId `
    --db-snapshot-identifier $SnapshotId `
    --db-instance-class db.t3.micro `
    --no-multi-az --no-publicly-accessible `
    --tags "Key=Purpose,Value=DRTest" "Key=AutoDelete,Value=true"

Write-Host "Run application smoke tests. Record actual RTO vs target: < 4 hours."
Write-Host "Delete test instance after validation:"
Write-Host "  aws rds delete-db-instance --db-instance-identifier $TestInstId --skip-final-snapshot"
```
